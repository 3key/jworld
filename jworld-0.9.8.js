	...
	$c.Sprite3d = function () 
	{
		...
		d.update = function () 
		{
			if($c.support) 
			{
				var t = this;
				var r = t.tgv;
				$c.cssProp(t.div, "transform", "translate3d("+t._x+"px,"+(t._y)+"px,"+t._z+"px) rotateY("+(t.getrotateY())+"deg) rotateX("+(t.getrotateX())+"deg) rotateZ("+(t.getrotateZ())+"deg) scale3d("+(t._scaleX)+","+(-t._scaleY)+","+(-t._scaleZ)+")");
						/*
						matrix currently buggy in mozilla only
